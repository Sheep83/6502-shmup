// ===========================================================================
// turrets.asm — static background-character turrets: art, combat and firing
// ===========================================================================
...
//
// PLACEMENT IS LEVEL DATA; THE ART IS ENGINE DATA. src/level1/stage_turrets.asm
// is part of the auto-generated level package and emits no bytes at all --
// TURRET_TOTAL plus two assembler lists, in world rows and columns derived as
// metatileRow * 4 + 1 and metatileCol * 4 + 1. The body glyphs are one shared
// static bitmap and live here.
//
...
// ---------------------------------------------------------------------------
// Well clear of terrain, which owns 96..167; the rest of the charset window is
...
// ---------------------------------------------------------------------------
// The hit-flash colour is deliberately NOT one of the pulse colours, so a hit
// is unmistakable against the idle animation. The hitbox is EXACTLY the
// 16-pixel body, not the 24 the enemy scan uses: a turret is smaller than a
// sprite and shooting one takes more aim.
.const TURRET_START_HEALTH  = 3         // three cannon hits to destroy
...
// The reward for destroying a turret, worth the same as an enemy. THE SCORE
// ITSELF IS NOT WIRED UP: there is no production score system yet. The value
// sits here so that whatever adds one has a place to hook -- see trtKills.
.const TURRET_SCORE_PER_KILL = 100      // defined, deliberately not wired

// ---------------------------------------------------------------------------
// FIRING
// ---------------------------------------------------------------------------
// The fire timer is PER TURRET, not global. A turret may only shoot from
// within a Y window, must hold fire when the ship is above or beside it
// (projectiles only fly downward, so a shot at a ship that is not below is
// wasted), and is refused entirely when the population gate is closed.
//
...
// row), so each turret fires at most once or twice per pass. That is both the
...
// so a turret adding a projectile to an already-busy world is spending exactly
// the scarcest budget there is. This is an ENCOUNTER-BUDGET POLICY, not a
// renderer limit: the renderer would schedule the projectile perfectly well,
// and refusing to create it is a decision about how the game should feel under
// load.
...
// ---------------------------------------------------------------------------
// ONE SHARED STATIC BODY, four codes, whatever the turret count -- and nothing
// ever changes them, so they are assembled at their final address with no
...
// 86.25, not 86, so an unfloored `turretRows.get(t) / METATILE_H == m` is false
// for every m and this table comes out entirely TURRET_NONE -- a silently empty
...
// ===========================================================================
// THE RESTORATION CONTRACT, which is what keeps destruction free:
//
...
//     dead  -> terrain only
//
// The overlay is applied ON TOP of a freshly decoded row and the authored
// terrain is never modified, so CLEARING turretAlive IS the whole of "put the
// terrain back" -- the next regeneration of either page produces the underlying
// terrain with nothing to repair. Destroying a turret therefore means (a) clear
// the byte and (b) let the DISPLAYED page catch up promptly rather than waiting
// for its next regeneration; see turretRestoreTick. There is no cached "ground
// codes" table, no repair path and no state that can go stale, because the
// decode is a pure function of the stage row and is recoverable at any time.
//
// TURRET_TOTAL AUTHORED TURRETS ARE THE WHOLE POOL. No slot streaming, no
// admission, no eviction, no cursor: every array below is indexed by the
// AUTHORED index directly, so a turret cannot change identity, cannot be
// evicted mid-flash, and needs no bitmap to keep it dead across re-admission.
* = $6e00 "turret state"
...
turretPaintRow:.fill TURRET_TOTAL, 0    // matrix row of the first body cell row
                                        ...
                                        // a body straddling an aperture edge is
                                        // still DRAWN. Colour must follow this
...
// single global counter, so two turrets on screen together pulse in step.
...
// STEP -- one frame in eight -- and a dense sprite frame has no room to redo
// it on the other seven.
trtTopLo:      .byte 0
...
trtKills:      .byte 0                  // turrets destroyed: the kill event a
                                        // future score system would read
...
// point, and omitting it silently produces an empty turretAtMetaRow table
...
//
// CALLED FROM gameFrame BEFORE collisionTick, and the order matters: positions
// must be derived from the PRESENTED origin, after the coarse step has settled
// and before the player's hitscan reads them.
//
// THE PRESENTED ORIGIN IS FREE. scrollTick runs at the END of gameFrame, so
// throughout the frame stageTopRow and scrollFine still describe the picture
// the player is looking at. Nothing has to be latched.
//
...
// without either side knowing what the other is made of.
//
// COMBAT VISIBILITY is "the full 16-pixel body is inside the aperture", which
// for rasters 55..247 means:
//
//     top    = 48 + fine + 8m  >=  55
...
// stage row alone -- so its colour must still be painted. Gate colour on the
// COMBAT predicate instead and a turret entering or leaving renders in flat
// terrain colour until its whole body is inside. turretPaintRow answers the
// drawing question; turretVisible answers the combat one.
//
// IT DOES NOT DERIVE ALL EIGHT, DELIBERATELY. Sixteen-bit subtraction with a
// modulo fold, eight times a frame, costs 983-1,246 cycles -- about sixteen
// raster lines, every frame -- for an answer that is "not on the page" for at
// least six of them, and that is enough to overrun a dense sprite frame.
//
// A page can only reach TURRET_SCAN_ROWS metatile rows, and turretAtMetaRow
// answers "is there a turret in this metatile row" in one indexed load. So the
// scan walks eight table entries and does the sixteen-bit work only for the
// turrets it finds, of which level 1 never has more than two at once: 250 to
// 480 cycles, with no cache and nothing to invalidate, because the table is
// assembled once and the scan is a pure function of stageTopRow.
// ---------------------------------------------------------------------------
turretWorldTick:
...
!swap:
    ...
    // transition. Blanking all eight unconditionally manufactures that
    // transition once every eight frames for a turret that has not moved at
...
//
// THE RASTER BOUNDS DO NOT CATCH THIS ON THEIR OWN, so the pair test is not
// redundant with them: at fine scroll 7 the derived top of such a body lands on
// exactly raster 55 and reads as fully visible.
// ---------------------------------------------------------------------------
turretAimTick:
    // ---- FLAT, AND DELIBERATELY NOT GATED ON shotFired --------------------
    // turretLogY and turretVisible have exactly one consumer -- traceTurretRay,
    // reached only on a frame that resolved a volley -- so skipping the other
    // seven frames looks like free savings. It is not: it saves 340 cycles on
    // seven frames and puts them ALL on the eighth, and since the weapon's
    // cadence is also eight frames the spike lands on the volley frame every
    // volley, in phase.
    //
...
!aim:
    ...
    // The rule is "a hundred CONSECUTIVE frames wholly on the aperture".
    // Spelling that as "reset the timer on every frame the turret is NOT
    // visible" would mean visiting the six turrets that are off the page every
    // frame, and turretFireTick deliberately does not visit them -- it leaves
    // in five cycles unless some turret is combat-visible at all, which is the
    // whole reason firing is affordable.
    //
    // Arming on the 0 -> 1 TRANSITION says the same thing from the other side,
    // costs six cycles on a path at most two turrets a frame reach, and is the
    // stricter reading: a body flickering at a band edge as the fine scroll
...
// and turrets must not break it: the ONLY cells ever written after init are
// the two-by-two of a turret that is on screen, and they are written back
...
turretPaintTick:
    ...
    // One phase for every turret, so two on screen together pulse in step
    // rather than each on its own clock.
    dec trtPulseTimer
    bne !phaseReady+
...
!phaseReady:
    ...
    //
    // Same shape as the page derivation above, and for the same reason: 640
    // cycles a frame with two turrets on screen is enough to overrun a dense
    // sprite frame.
    lda trtRepaint
    ora trtFlash
    bne !work+
...
!work:
    ...
    // ---- decay the hit flash ---------------------------------------------
    // UNCONDITIONALLY for an alive turret, NOT only while it is combat-visible.
    // A turret can only be HIT while visible, so the two rules differ in one
    // case: a turret scrolling off mid-flash. Gate the decay on visibility and
    // its timer freezes, and the flash reappears when the turret comes back.
    // Decaying always costs the same four cycles and cannot go stale.
    lda turretAlive,x
...
//
// THIS EXTENDS THE ENEMY SCAN'S RESULT, it does not run a second competition.
// There is still ONE winner per ray and one place that decides it.
//
// THE ARBITRATION RULE, AND THE TIE. The test below sends both `bcc` AND `beq`
// to the reject path -- a turret must be STRICTLY nearer than the incumbent,
// where the enemy scan's own test replaces an equal one. So:
//
//     nearest wins, and AN ENEMY WINS AN EXACT TIE.
//
// It also means this routine can never WEAKEN the result: with no turret in
// the ray's path it either rejects every turret and returns, or replaces an
// incumbent it strictly beat.
//
// THE HITBOX IS SIXTEEN PIXELS, NOT TWENTY-FOUR. A turret is a 2x2 character
// body and its hitbox is exactly that body, where an enemy's is the full
// 24-pixel sprite width.
// ---------------------------------------------------------------------------
traceTurretRay:
...
// level. Nothing here walks the authored turrets unless one of them genuinely
...
!fire:
    // THE TIMER RELOADS BEFORE THE ELIGIBILITY TESTS, and that order is a
    // gameplay decision rather than an oversight: a shot the geometry refuses
    // costs a whole interval instead of being retried every frame until it
    // lands. Reload afterwards and a turret held just out of position runs
    // these tests fifty times a second and fires the instant it can.
    lda #TURRET_FIRE_INTERVAL
    sta turretFireTimer,x
    ...
    bne !next+                          // the bolt would pass through it anyway
...
// not depend on how busy the frame was.
//
